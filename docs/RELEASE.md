...
The internal project version is `0.1.1`. The release workflow accepts exactly
the engineering-candidate tag `v0.1.1-rc.1`.
...
If another candidate is needed, first update and review the workflow for a new
tag such as `v0.1.1-rc.2`; never rewrite a published tag.

## What the workflow does

`.github/workflows/release.yml` runs only when `v0.1.1-rc.1` is pushed.
...
2. On macOS, create universal2 arm64+x86_64 AU and VST3 bundles with minimum
   ...
   Studio generator.
3. Execute the offline behavioral tests on both platforms.
...
`v0.1.1-rc.1` does not exist locally or remotely, then annotate the exact merge
commit rather than whichever commit happens to be checked out:

    set -euo pipefail
    candidate_tag=v0.1.1-rc.1
    ...
    git tag -a "$candidate_tag" "$approved_merge_sha" \
      -m "Granular Freeze v0.1.1 release candidate 1"
...
- VST3 and AU DAW loading, Freeze/Pitch/crossfade behavior, session restore,
  and listening checks without unresolved glitches
...
- No draft release: check that the exact tag is `v0.1.1-rc.1` and that both
